...
function startServer(
  ...
  onSlackConnected?: (slackUserId: string) => Promise<void>
): void {
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (slackWorkspaceAuth && req.url?.startsWith("/oauth/slack/")) {
      const handled = await handleSlackWorkspaceRoutes(req, res, slackWorkspaceAuth, onSlackConnected);
      if (handled) return;
    }
    res.writeHead(404);
    res.end();
...
async function main(): Promise<void> {
  ...
  console.log(
    `[boot] connections: ${credentials.enabled ? "enabled (/connect)" : "disabled (set CREDENTIAL_ENC_KEY)"}` +
      ...
      `; slack workspace oauth: ${slackWorkspaceAuth ? "enabled" : "disabled"}`
  );
  if ((googleAuth || granolaAuth || slackWorkspaceAuth) && !config.publicUrl) {
    console.warn(
  ...
  let onSlackConnected: ((slackUserId: string) => Promise<void>) | undefined;
  ...
  if (config.slack.configured) {
    ...
    if (config.anthropic.configured) {
      ...
      resolvedAgent = await Agent.create(credentials, undefined, app.client, slackWorkspaceAuth);
    ...
    onSlackConnected = async (slackUserId) => {
      await app.client.chat.postMessage({
        channel: slackUserId,
        text: "✅ Slack workspace connected! When you ask me to send a DM, I can now also reach people in your connected workspace.",
      });
    };
  ...
  startServer(googleAuth, granolaAuth, slackWorkspaceAuth, onGoogleConnected, onGranolaConnected, onSlackConnected);
