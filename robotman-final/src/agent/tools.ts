...
export interface ToolContext {
  ...
  /** Slack Workspace OAuth auth provider; null when not configured. */
...
const connectIntegration: AgentTool = {
  ...
  async run(input, ctx) {
    ...
    if (SLACK_OAUTH_ALIASES.has(provider)) provider = SLACK_PROVIDER;
    requestConnect(ctx, provider);
    const isOAuthProvider = provider === "google" || provider === "granola" || provider === SLACK_PROVIDER;
    return {
      text: isOAuthProvider
        ? `Connection flow for "${provider}" started. The app will show a Connect button — the user clicks it to sign in with their ${provider} account. Tell them to click it.`
        : `Connection flow for "${provider}" started. The app will show the user a Connect button to securely enter their site and API key. Tell them to click it.`,
      citations: [],
    };
  },
};

/** Provider names that trigger the Slack workspace OAuth flow. */
export const SLACK_OAUTH_ALIASES = new Set([
  "slack",
  "slack-workspace",
  "slackworkspace",
]);
...
const slackSendDm: AgentTool = {
  def: {
    name: "slack_send_dm",
    description:
      "Send a direct message to any Slack user by email address or display name. " +
      "Works in two modes: (1) the bot's own workspace (default); (2) a connected external " +
      "workspace if the user has linked one via 'Connect with Slack'. " +
      "The requesting user's real name is automatically prepended to the message so the " +
      "recipient knows who it's from — e.g. '*[From Sarah Jones]:* The interview is confirmed.' " +
      "Set `attribute` to false to suppress prepending. " +
      "Always confirm the message content before sending unless the user says 'just send it'.",
    input_schema: {
      type: "object",
      properties: {
        to: {
          type: "string",
          description: "Recipient's email address (preferred) or Slack display name / real name.",
        },
        message: {
          type: "string",
          description: "Message text (Slack mrkdwn supported). Do NOT include attribution yourself — it is prepended automatically.",
        },
        attribute: {
          type: "boolean",
          description: "Prepend the requesting user's name. Default true. Set false only if the user explicitly asks.",
        },
        workspace: {
          type: "string",
          enum: ["own", "connected"],
          description: "Which workspace to send in. 'own' = the bot's workspace (default); 'connected' = an OAuth-linked external workspace.",
        },
      },
      required: ["to", "message"],
    },
  ...
  async run(input, ctx) {
    ...
    const attribute = input.attribute !== false; // default true
    const useConnected = input.workspace === "connected";
    if (!to || !rawMessage) return { text: "Need both a recipient and a message.", citations: [] };

    // Build the attributed message.
    const senderName = ctx.user.displayName ?? ctx.user.email ?? ctx.user.slackUserId;
    const message = attribute ? `*[From ${senderName}]:* ${rawMessage}` : rawMessage;

    // ── Try connected (external) workspace ───────────────────────────────────
    if (useConnected || ctx.slackWorkspaceAuth) {
      const botToken = ctx.slackWorkspaceAuth
        ? await ctx.slackWorkspaceAuth.getBotToken(ctx.user.slackUserId)
        : null;

      if (botToken) {
        const wsUser = await findWorkspaceUser(botToken, to);
        if (!wsUser) {
          return {
            text: `Could not find "${to}" in the connected workspace. Try their exact email or connect the workspace (/connect → Connect with Slack).`,
            citations: [],
          };
        }
        const result = await sendWorkspaceDm(botToken, wsUser.id, message);
        if (!result.ok) {
          return { text: `Connected-workspace DM failed: ${result.error ?? "unknown error"}`, citations: [] };
        }
        return {
          text: `✅ DM sent to ${wsUser.realName} (${to}) in connected workspace${attribute ? ` as *${senderName}*` : ""}.`,
          citations: [],
        };
      }

      if (useConnected) {
        // User explicitly asked for connected workspace but it isn't set up.
        requestConnect(ctx, SLACK_PROVIDER);
        return {
          text: "No Slack workspace is connected yet. Click the Connect with Slack button to link one.",
          citations: [],
        };
      }
    }

    // ── Fall back to bot's own workspace ─────────────────────────────────────
    if (!ctx.slackClient) {
    ...
    if (!channelId) {
      return {
        text: `Could not find a Slack user matching "${to}". Try their exact email address.`,
        citations: [],
      };
    }

    try {
      await ctx.slackClient.chat.postMessage({ channel: channelId, text: message, unfurl_links: false });
      return {
        text: `✅ DM sent to ${to}${attribute ? ` (attributed to *${senderName}*)` : ""}: "${rawMessage.slice(0, 100)}${rawMessage.length > 100 ? "…" : ""}"`,
        citations: [],
      };
    } catch (err) {
      return { text: `Failed to send DM: ${err instanceof Error ? err.message : String(err)}`, citations: [] };
    }
  },
};
...
/** Pending calendar invite awaiting user confirmation before sending. */
export interface CalInviteDraft {
  summary: string;
  start: string;
  end: string;
  attendees: string[];
...
const gcalCreateEventTool: AgentTool = {
  def: {
    ...
    input_schema: {
      type: "object",
      properties: {
        summary: { type: "string", description: "Event title (e.g. 'Interview — Jane Smith / Backend Engineer')." },
        start: { type: "string", description: "Start datetime, ISO 8601 with timezone offset (e.g. '2026-06-25T14:00:00-07:00')." },
        end: { type: "string", description: "End datetime, ISO 8601 with timezone offset." },
        attendees: {
          type: "array",
          items: { type: "string" },
          description: "Email addresses of all attendees (include the host and any interviewers).",
        },
  ...
  async run(input, ctx) {
    ...
    const addMeetLink = input.add_meet_link !== false; // default true
    const confirmed = input.confirmed === true;

    if (!summary || !start || !end) {
      return { text: "Please provide a title, start time, and end time for the event.", citations: [] };
    }

    // Stage for confirmation unless already confirmed.
    if (!confirmed) {
      const attendeeList = attendees.length ? attendees.join(", ") : "no attendees specified";
      const meetNote = addMeetLink ? "A Google Meet link will be generated." : "No Meet link.";
      return {
        text:
          `📅 *Ready to send this invite — please confirm:*
` +
          `*Title:* ${summary}
` +
          `*Start:* ${start}
...
` +
          `Reply "yes" or "send it" to confirm, or let me know if anything needs changing.`,
        citations: [],
      };
    }
    ...
    try {
      const { event, status, error } = await gcalCreateEvent(token, {
        summary,
        start,
        end,
        attendees,
      ...
      if (!event) {
        return { text: `Failed to create event (HTTP ${status}): ${error ?? "unknown error"}`, citations: [] };
      }
...
*Event link:* ${event.htmlLink}`,
        citations: [{ label: event.summary, url: event.htmlLink, source: "gcal" }],
      };
    } catch (err) {
      return { text: `Calendar error: ${err instanceof Error ? err.message : String(err)}`, citations: [] };
    }
